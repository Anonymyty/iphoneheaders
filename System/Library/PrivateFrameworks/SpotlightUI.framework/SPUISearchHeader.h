/*
* This header is generated by classdump-dyld 1.0
* on Wednesday, January 25, 2017 at 5:54:59 PM Eastern Standard Time
* Operating System: Version 9.3.3 (Build 13G34)
* Image Source: /System/Library/PrivateFrameworks/SpotlightUI.framework/SpotlightUI
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <SpotlightUI/SpotlightUI-Structs.h>
#import <UIKit/UIView.h>

@protocol SPUISearchHeaderDelegate;
@class UIButton, UIView, NSArray, UITextField, NSLayoutConstraint;

@interface SPUISearchHeader : UIView {

	UIButton* _cancelButton;
	UIEdgeInsets _cancelMargins;
	UIEdgeInsets _margins;
	UIButton* _voiceButton;
	UIView* insetContainer;
	BOOL _shouldHideCancel;
	NSArray* _insetConstraints;
	BOOL _inputFieldHasFocus;
	UITextField* _searchField;
	id<SPUISearchHeaderDelegate> _delegate;
	NSLayoutConstraint* _cancelButtonTrailingConstraint;

}

@property (nonatomic,retain,readonly) UITextField * searchField;                     //@synthesize searchField=_searchField - In the implementation block
@property (assign,nonatomic) id<SPUISearchHeaderDelegate> delegate;                  //@synthesize delegate=_delegate - In the implementation block
@property (assign) BOOL inputFieldHasFocus;                                          //@synthesize inputFieldHasFocus=_inputFieldHasFocus - In the implementation block
@property (retain) NSLayoutConstraint * cancelButtonTrailingConstraint;              //@synthesize cancelButtonTrailingConstraint=_cancelButtonTrailingConstraint - In the implementation block
+(id)barTintColor;
+(id)generateImageNamed:(id)arg1 withColor:(id)arg2 forClass:(Class)arg3 isKitImage:(BOOL)arg4 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)setDelegate:(id<SPUISearchHeaderDelegate>)arg1 ;
-(void)dealloc;
-(id)hitTest:(CGPoint)arg1 withEvent:(id)arg2 ;
-(void)layoutSubviews;
-(CGSize)sizeThatFits:(CGSize)arg1 ;
-(void)traitCollectionDidChange:(id)arg1 ;
-(id<SPUISearchHeaderDelegate>)delegate;
-(id)nextResponder;
-(void)textFieldDidBeginEditing:(id)arg1 ;
-(void)textFieldDidEndEditing:(id)arg1 ;
-(UITextField *)searchField;
-(void)_cancelButtonPressed;
-(void)_updateConstraints;
-(void)updatePlaceholder;
-(void)updateMicrophoneStatus;
-(void)handleDictationButtonPress:(id)arg1 ;
-(BOOL)isDictationAvailable;
-(void)setCancelButtonTrailingConstraint:(NSLayoutConstraint *)arg1 ;
-(NSLayoutConstraint *)cancelButtonTrailingConstraint;
-(BOOL)shouldHideCancel;
-(void)setCancelButtonHidden:(BOOL)arg1 animated:(BOOL)arg2 ;
-(void)updateCancelButtonVisibilityAnimated:(BOOL)arg1 ;
-(void)setIsCapturing:(BOOL)arg1 ;
-(void)showKeyboardAndSelectQueryStringIfPossible;
-(BOOL)inputFieldHasFocus;
-(void)setInputFieldHasFocus:(BOOL)arg1 ;
@end

